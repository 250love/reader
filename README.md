...
## 本阶段新增功能：批量 PDF 导入与拖拽上传

文献库页面现在支持通过点击选择或拖拽方式一次导入多个 PDF 文件。展开“添加文章”后，可以在“导入 PDF”区域选择一个或多个 PDF，或直接把文件拖到上传区域。系统会在上传前显示待上传文件列表，并为每个文件展示等待上传、上传中、上传成功或上传失败状态。

批量导入会复用现有单文件上传接口逐个上传 PDF，并在上传成功后自动创建文献记录。单个文件失败不会影响其他文件继续上传，失败文件可以单独重试。非 PDF 文件、空文件、后端上传失败和登录失效都会显示明确的失败提示。批量导入创建的文献会沿用当前选中的文件夹；全部处理完成后会自动刷新文献列表。

## 本阶段新增功能：白天 / 黑夜主题切换

系统新增全局主题切换能力，支持白天模式和黑夜模式。用户可以在顶部导航右侧使用固定位置的主题按钮一键切换，也可以在账号设置的“外观设置”中选择主题。主题选择会保存在浏览器本地，刷新页面后仍然生效。

主题系统基于全局 CSS 变量实现，当前已覆盖文献库 / Dashboard、PDF 阅读器、引用助手、设置页和学术 AI 页面等主要模块。暗色模式针对学术 AI、引用助手和 PDF 阅读器做了高对比度适配：主要面板使用深色背景，长文本阅读区域和表单输入区域按场景使用亮底深字，确保标题、说明文字、对话内容、引用结果、模型设置和关闭按钮等元素在黑夜模式下仍然清晰可读。后续新增页面可继续复用这些变量，便于扩展主题色、护眼模式和阅读字体设置。

## 本阶段新增功能：帮助中心与教学使用指南

系统新增全局帮助入口，用户可以在论文管理、PDF 阅读器、引用助手、学术 AI 和设置页等主要页面随时打开帮助中心。帮助中心采用类似设置面板的分栏弹窗设计，左侧按功能分类，右侧展示对应的操作说明、注意事项和教学使用建议；弹窗支持点击遮罩、关闭按钮或 Esc 键关闭。

当前帮助内容覆盖快速开始、PDF 导入与文献管理、论文阅读器、高亮笔记与翻译、引用助手、学术 AI 助手、模型 Provider 配置、主题与外观、教学与课程项目使用建议和常见问题。帮助弹窗会跟随白天 / 黑夜主题自动适配，便于新用户快速了解系统完整使用流程。
