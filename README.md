...
学术 AI 页面已从传统表单式布局升级为更接近 AI 工作台的对话式布局。新版页面采用左侧可折叠工具栏与主对话区结构，将论文上下文、任务模板、模型设置和更多工具入口收纳到侧边栏中，主区域专注展示用户提问与 AI 回复。底部输入框采用类似 ChatGPT 网页端的问答体验，并为后续全文搜索、图表与表格、随笔和阅读计划等能力预留扩展入口。
